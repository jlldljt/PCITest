...
BOOL CDlgPriview1::OnInitDialog()
{
  ...
  m_RectTracker.m_rect.SetRect(g_npc_inf.left* rect.Width() / gclsImgRcg.g_stu_square.nBMPW
...
void CDlgPriview1::OnPaint()
{
  ...
  g_dlgCamera->GetDlgItem(IDC_PREVIEW)->GetClientRect(&rect_camera);
  dc.TransparentBlt/*BitBlt*/(wnd_rc.left, wnd_rc.top, wnd_rc.Width(), wnd_rc.Height(), g_dlgCamera->mDCMem, 0, 0, rect_camera.Width(), rect_camera.Height(), SRCCOPY);
  // 
...
void CDlgPriview1::OnDestroy()
{
  ...
  g_npc_inf.left = (rect.left * gclsImgRcg.g_stu_square.nBMPW / (wnd_rc.Width()));
  g_npc_inf.top=(rect.top * gclsImgRcg.g_stu_square.nBMPH / (wnd_rc.Height()));
  g_npc_inf.right=(rect.right * gclsImgRcg.g_stu_square.nBMPW / (wnd_rc.Width()));
