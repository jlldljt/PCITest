...
BOOL CBERDlg::SYSInit()
{
	...
	g_dlgDebug->SetWindowPos(NULL, initRect.left, initRect.top, initRect.Width(), initRect.Height(), SWP_SHOWWINDOW);
    ...
    WritePrivateProfileString(_T("范围设置"), _T("left"), _T("0"), gstuPathInf.csPathIni);
    WritePrivateProfileString(_T("范围设置"), _T("top"), _T("0"), gstuPathInf.csPathIni);
    WritePrivateProfileString(_T("范围设置"), _T("right"), _T("0"), gstuPathInf.csPathIni);
    WritePrivateProfileString(_T("范围设置"), _T("bottom"), _T("0"), gstuPathInf.csPathIni);
		for(int i=0;i<30;i++)
		{
...
BOOL CBERDlg::DBGInit()
{
  ...
  GetPrivateProfileString(_T("范围设置"), _T("left"), NULL, strbuff, 80, gstuPathInf.csPathIni);
  strValue = strbuff;
  g_npc_inf.left = _wtof(strValue.GetBuffer(strValue.GetLength()));
  strValue.ReleaseBuffer();

  GetPrivateProfileString(_T("范围设置"), _T("top"), NULL, strbuff, 80, gstuPathInf.csPathIni);
  strValue = strbuff;
  g_npc_inf.top = _wtof(strValue.GetBuffer(strValue.GetLength()));
  strValue.ReleaseBuffer();

  GetPrivateProfileString(_T("范围设置"), _T("right"), NULL, strbuff, 80, gstuPathInf.csPathIni);
  strValue = strbuff;
  g_npc_inf.right = _wtof(strValue.GetBuffer(strValue.GetLength()));
  strValue.ReleaseBuffer();

  GetPrivateProfileString(_T("范围设置"), _T("bottom"), NULL, strbuff, 80, gstuPathInf.csPathIni);
  strValue = strbuff;
