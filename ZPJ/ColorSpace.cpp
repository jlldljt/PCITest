...
__forceinline void YUVToRGB32_Two(TARGB32* pDst, const TUInt8 Y0, const TUInt8 Y1, const TUInt8 U, const TUInt8 V)
{
    ...
    pDst->b	= (unsigned char)border_color( ( Ye0 + Ue_blue ) >>16 );
    pDst->g	= (unsigned char)border_color( ( Ye0 + UeVe_green ) >>16 );
    pDst->r	= (unsigned char)border_color( ( Ye0 + Ve_red ) >>16 );
    pDst->a	= 255;

	++pDst;

	pDst->b	= (unsigned char)border_color( ( Ye1 + Ue_blue ) >>16 );
    pDst->g	= (unsigned char)border_color( ( Ye1 + UeVe_green ) >>16 );
    pDst->r	= (unsigned char)border_color( ( Ye1 + Ve_red ) >>16 );
    pDst->a	= 255;
}
...
bool DECODE_UYVY_TO_RGB32(const TUInt8 *pUYVY, const TPicRegion &DstPic)
{
	if (NULL == pUYVY)
	{
		AfxMessageBox (_T("DECODE_UYVY_TO_RGB32() error, NULL == pUYVY\n"));
		return false;
	}

	if (NULL == DstPic.pdata)
	{
		AfxMessageBox (_T("DECODE_UYVY_TO_RGB32() error, NULL == DstPic.pdata\n"));
		return false;
	}
...
bool DECODE_RGB_TO_BGRA(const unsigned char *pRGB, const TPicRegion &DstPic)
{
	...
	for (register int i=0 ; i<(int)RGBcount ; i++)
	{
...
bool DECODE_RGB_TO_GRAY(const unsigned char *pRGB, const TPicRegion &DstPic)
{
 	DWORD RGBcount = DstPic.width * DstPic.height;
	for (register int i=0 ; i<(int)RGBcount ; i++)
	{
		double gray=pRGB[i*3];//pRGB[i*3]*0.3+pRGB[i*3+1]*0.59+pRGB[i*3+2]*0.11;
		DstPic.pdata[i].r = (unsigned char)gray;
		DstPic.pdata[i].g = (unsigned char)gray;
		DstPic.pdata[i].b = (unsigned char)gray;
		DstPic.pdata[i].a = 0;
...
bool DECODE_THRESHOLD(const TPicRegion &DstPic,const TPicRegion &DstPic1,BYTE * lpGray,int x)
{
	int y=255;
	DWORD RGBcount = DstPic1.width * DstPic1.height;
	for (register int i=0 ; i<DstPic1.height ; i++)
		for(register int j=0; j<DstPic1.width;j++)
		{
		
			if(DstPic.pdata[i*DstPic1.width+j].r>x)
			{
				DstPic1.pdata[i*DstPic1.width+j].r = y;
				DstPic1.pdata[i*DstPic1.width+j].g = y;
				DstPic1.pdata[i*DstPic1.width+j].b = y;
				DstPic1.pdata[i*DstPic1.width+j].a = 0;
				lpGray[i*DstPic1.width+j]=1;
			}
			else
			{
				DstPic1.pdata[i*DstPic1.width+j].r = 0;
				DstPic1.pdata[i*DstPic1.width+j].g = 0;
				DstPic1.pdata[i*DstPic1.width+j].b = 0;
				DstPic1.pdata[i*DstPic1.width+j].a = 0;
				lpGray[i*DstPic1.width+j]=0;
			}
		}
	return true;
}
bool DECODE_THRESHOLD2(const TPicRegion &DstPic,const TPicRegion &DstPic1,int x)
{
	int y=255;
	DWORD RGBcount = DstPic1.width * DstPic1.height;
	for (register int i=0 ; i<DstPic1.height ; i++)
...
//图片二值化
bool PhotoThreshold(int height,int width,BYTE *lpGray,int m_VGAthreshold)
{
	for(int i=0;i<height;i++)
		for(int j=0;j<width;j++)
		{
			if(lpGray[i*width+j]> m_VGAthreshold )//二值化
			{
				lpGray[i*width+j]=1;
