...
#define DEG_TO_SEC(deg) ((deg)*3600)
...
public:
 ...
 // void JudegWhichSortDegree(double sec, int *sort_sec, int step_sec, int sort_num, int &which);
  //°
  BOOL GetD1D2DM(double & D1, double & D2, double & DM, int pluse_num);
  BOOL GetD1D2DM(double &D1, double &D2, double &DM, int pluse_num, double factor_h, double factor_w);
  BOOL SortSpike(int pluse_num);
  // 计算原始光轴/原始电轴/u轴g，°
  void CalcDegree0(const double D1, const double D2, const double DM, double & laser0, double & phi0, double &u_g);
  // 计算修正光轴/电轴，°
  void CalcDegree1(double laser0, double phi0, struct tagSinParam sinparam, double laser1, double phi1);
  // 计算等效角，°
  void CalcEquAngle(double laser0, double phi0, double equ_phi, double equ_factor, double & equ);
