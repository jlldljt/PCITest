...
CDiIntCounterSnap::CDiIntCounterSnap()
{
  ...
  {
    m_counter.counter_laser[0][i] = 50;
    m_counter.counter_laser[1][i] = 100;
    m_counter.counter_laser[2][i] = 50 + i;
    m_counter.counter_laser[3][i] = 100 + i;
  }
...
void CDiIntCounterSnap::DIIntLaserSin(void)
{
  ...
  double fparam;
  for (int i = 0; i < SIN_CNT_NUM; i++) {
    m_card->ReadT0(i, m_counter.counter_laser[i][m_counter.index], fparam);
  }
#endif
  if (m_counter.index++ >= LASER_SIN_NUM - 1)
  {
    ...
    for (int i = LASER_SIN_NUM - 1; i > 0; i--) {
      for (int j = 0; j < SIN_CNT_NUM; j++) {
        m_counter.counter_laser[j][i] -= m_counter.counter_laser[j][i - 1];
      }
	  m_counter.fit[0][i] = (m_counter.counter_laser[1][i] + m_counter.counter_laser[0][i]+m_counter.counter_laser[3][i] + m_counter.counter_laser[2][i])/4;
    }
#endif
...
void CDiIntCounterSnap::DIIntXRayOneShot(void)
{
  ...
  if (m_counter.start) {
    ...
    for (int i = 0; i < XRAY_CNT_NUM; i++) {
      m_card_add->StopT0(i);
      m_card_add->StartT0(i, m_device_add, 0, 0);//out6
    }
#endif
    m_counter.flag = 1;
  }
  ...
  //读取计数器
  for (int i = 0; i < XRAY_CNT_NUM; i++) {
    m_card_add->ReadT0(i, m_counter.counter_xray[i][m_counter.index], fparam);
  }
#endif
  if (m_counter.index++ >= XRAY_ONESHOT_NUM - 1)
  {
    ...
    for (int i = XRAY_ONESHOT_NUM - 1; i > 0; i--) {
      for (int j = 0; j < XRAY_CNT_NUM; j++) {
        m_counter.counter_xray[j][i] -= m_counter.counter_xray[j][i - 1];
      }
    }
    ...
    {
      for (int j = 0; j < XRAY_CNT_NUM; j++) {
        m_counter.fit[0][i] += m_counter.counter_xray[j][i];
      }
    }
#endif
...
////////////////////////////////////////////////////////////////////////
int CDiIntCounterSnap::BindCard(int device, CPCIBase * card, CBoardView *viewBoard, int device_add, CPCIBase *card_add)
{
  //if (device < 0 || !card)
  //  return -1;
...
{
  if (m_device_add < 0 || !m_card_add)
    return -1;
  if (m_card_add->StartDi(channel, m_device_add, 0, 0, UserFuncXRayOneShot, this)) {
    m_channel_add = channel;
    return 0;
  }
...
int CDiIntCounterSnap::StopDiIntAdd()
{
  if (m_channel_add < 0 || !m_card_add)
    return -1;
  if (m_card_add->StopDi(m_channel_add)) {
    m_channel_add = -1;
    return 0;
  }
  return -1;
