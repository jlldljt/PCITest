...
public:
	...
	//com其他操作相关
	UINT SendCharToCom(const char* comStr,bool sucDisable);		//发送字符串
	UINT SendCharToComNOID(const char* comStr);	//发送字符串沒有id加。主要用于初始化操作，防止id被改掉
	UINT RecvCharFromCom();						//接收字符串
